...
def estimate_pre_retirement_tax_rate(income, retirement_contribution, filing='single'):
    single_tax_rates, married_tax_rates = rate_table()
    std_deduction_married = 30000
    std_deduction_single = 15000
    if filing == 'single':
        taxable_income = income - retirement_contribution - std_deduction_single
        tax_rates = single_tax_rates
    elif filing == 'married':
        taxable_income = income - retirement_contribution - std_deduction_married
    ...
    if taxable_income > 0:
        ...
        for i, tax_rate in tax_rates.iterrows():
            if taxable_income >= tax_rate['To$']:
                ...
                print(tax_amt)
                break
        effective_tax_rate = int((tax_amt / income)*100)
    else:
        effective_tax_rate = 0
    return effective_tax_rate

def estimate_retirement_tax_rate(total_expense, ssn_earning, filing='single'):
